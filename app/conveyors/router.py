...
async def get_conveyors(session: AsyncSession = Depends(get_db)):
    return await ConveyorDAO.find_all(session)


@router.post("/create", response_model=ConveyorRead)
async def create_conveyor(data: ConveyorCreate, session: AsyncSession = Depends(get_db), user: User = Depends(get_current_admin)):
    return await ConveyorDAO.create(session, name=data.name, description=data.description)
